...
$ShearPhoto["config"] = array("proportional" => 0.78, //比例截图，如果你是动态比例，请设为false，false时就不会验证JS端的比例了。如果为数字时，JS端也要相应设置哦，不然系统会给你抱出错误,不设比例填0，如填比例 ：3/4  代表宽和高的比例是3/4(3除以4的意思懂吗，菜菜，你可直接填0.75，没错)
...
array(477, true, "big"), //@参数1要生成的宽 （高度不用设，系统会按比例做事），    @参数2：是否为该图加水印，water参数要有水印地址才有效true或false  @参数3：图片后面添加字符串 （用以区分其他截图名称),填写字符串，不要含中文，不然能又鸡巴痛了 ，不定义的话默认为“0”

array(150, true, "centre"), //centre @参数1要生成的宽 （高度不用设，系统会按比例做事），   @参数2：是否为该图加水印，water参数要有水印地址才有效true或false  @参数3：图片后面添加字符串 （用以区分其他截图名称),填写字符串，不要含中文，不然能又鸡巴痛了，不定义的话默认为"0"

array(70, true, "small")//你可以继续增加多张照片,也可以删除不要的，默认是3张哦
), "water" => "false", //只接受PNG水印../images/waterimg2.png，当然你对PHP熟练，你可以对主程序进行修改支持其他类型水印,不设就"water"=>false
...
"saveURL" => ShearURL . images . DIRECTORY_SEPARATOR, //截好后的图片。储存的目录位置，后面不要加斜杠，系统会自动给补上！不要使用中文
