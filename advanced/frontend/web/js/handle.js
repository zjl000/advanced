...
window.ShearPhoto.MINGGE(function() {
	//██████████重要设置████████████████
	var relativeUrl = ""; //你不要在后面加斜杠，系统会自动给你加上斜杠，不信看下面！   index.html的JS引用路径自己改，很简单的说
	...
	Shear.config({
		...
		proportional: [0.78, <!--截框的宽高比例（宽除以高的比例值，这个设置其实就是1，对！你可以直接写1  如填3/4 那么就是0.75的比例,不设比例请设为0，注意更改比例后，后端也要进行相应设置，否则系统会给你抱出错误-->
			/*
	...
	Shear.complete = function(serverdata) { //截图成功完成时，由shearphoto.php返回数据过来的成功包
			...
			var length = serverdata.length, div,
				creatImg, p;
			for (var i = 1; i < length; i++) {
				...
				if(i == 1){
					p.innerHTML = '477 x 611';
				}else{
					p.innerHTML = '150 x 192';
				}
				complete.appendChild(div);	
				div.appendChild(creatImg);	
				div.appendChild(p);

				// createImg = document.createElement('img'); //原插件代码
				// complete.appendChild(createImg); //原插件代码
				creatImg.src = this.arg.relativeUrl + serverdata[i]["ImgUrl"];
			}
			// alert(serverdata[0]['ImgUrl']);//第一张图片路径
			...
			//新增部分
			var revImg = document.getElementById('revImg'); //封面图片
			var cover = document.getElementById('cover'); //隐藏域
			var bigUrl = serverdata[1]['ImgUrl']; //截取的大图路径
			var bigName = serverdata[0]['ImgName']; //截取的大图名
			var bg = document.getElementById('bg'); //背景
			var uploadFile = document.getElementById('uploadFile'); //上传图片框
			// alert(cover);
			// alert(bigName);
	...
	try {
		webcam.init_config = {
				...
				postargs: {
					mingge: "shearphoto我爱你",
					shearphoto: "shearphoto好"
				} //示例传入POST参数到后端。后端示例$_POST['XXXX'] 接收！如果！ 不需要传参数，请把postargs删除，需填参数请保证参数格式
			},
		...
		console.log("拍照对象检测不到，默认你抱弃这个功能，错误代码：" + e);
	}
	/*----------------------------------拍照-----------拍照功能在这设置-----------结束----------------------------------------------------------------------*/
